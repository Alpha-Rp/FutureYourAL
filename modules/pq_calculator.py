...
def calculate_risk_reward(pq_score, skills_matched, years_of_experience):
    ...
    :return: A string with the risk-reward analysis.
    """
    # Risk-Reward logic based on PQ Score and skills
    if pq_score >= 75 and skills_matched >= 4:
        return "Safe Candidate: Well-matched, but with low growth potential."
    elif pq_score < 50 or years_of_experience < 2:
        return f"High-Risk, High-Reward: {100 - pq_score}% match now but has the potential to exceed expectations with training."
    else:
        return f"Balanced Candidate: {pq_score}% match with a {100 - pq_score}% chance of exceeding expectations within a year."
